...
This guide outlines the steps to initialize the **MatekWebApp** project with React, Tailwind CSS v4, shadcn/ui, and a
...
**Important:** If Vite installs React 19 by default, you might encounter dependency conflicts with `lucide-react` or
other libraries. It is recommended to downgrade to React 18 in `package.json` if you see `ERESOLVE` errors.
...
shadcn/ui Blocks are pre-built sections (like dashboards, authentication forms, hero sections) that you can copy and
...
3. **Install Dependencies:** The block description will list the components it uses (e.g., `Card`, `Button`, `Input`,
   `DropdownMenu`). Install them using the CLI:
   ```bash
...
2. **Install Backend Dependencies:**
   Run this in the project root. We are adding these to the main `package.json` for simplicity, but they will be used by
   the server code.
   ```bash
...
Create a file named `server/database.js`. This script will connect to the SQLite database and initialize the tables if
they don't exist.
